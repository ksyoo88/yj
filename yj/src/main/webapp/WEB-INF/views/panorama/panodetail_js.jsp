	...
	$(".btn-save").click(function() {
		...
		if(daycnt>=2){
			for(var i=2;i<=daycnt;i++){
				content+='<input type="text" id="locaform'+i+'" name="locaform">'
				content+='<input type="text" id="memoform'+i+'" name="memoform">'
				
				$(content).insertBefore('#submitBtn');
		...
		$.ajax({
				...
				success: function(result) {
					console.log("제목저장-");
					console.log(result.panoseq);
					panoseq=result.panoseq;
		...
		var memo=$("#Memo"+i+" textarea").val();
		var loca=$("#newtemp"+i+" #location").val();
			$.ajax({
				url:"savePanoMemo.do",
				data:{memo:memo,
						day:i,
				...
				success: function(result) {
					cosole.log("메모저장-");
					console.log(result.panodayseq);
					panodayseq=result.panodayseq;
			...
			$.ajax({
				url:"savePanoDayPhoto.do",
				data:{panodayseq:panodayseq,day:i},
				type:"post",
				dataType:"json",
				success: function(result){
					console.log("사진저장완료")
